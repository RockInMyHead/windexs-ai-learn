...
app.get('/api/chat/:courseId/history', authenticateToken, (req, res) => {
  try {
    ...
    const messages = db.prepare(`
      ...
      FROM chat_messages
      WHERE user_id = ? AND course_id = ? AND (message_type IS NULL OR message_type != 'voice')
      ORDER BY created_at DESC
      LIMIT 15
    `).all(userId, courseId);
...
app.post('/api/tts', authenticateToken, async (req, res) => {
  try {
    ...
    if (!openai) {
      console.log('⚠️ OpenAI client not initialized, using TTS stub');
      // Return a simple silent MP3 file for development/testing
