...
### 🔒 Настройка прокси (для регионов с ограничениями)
Если вы получаете ошибку `unsupported_country_region_territory`, настройте прокси:

1. **Скопируйте настройки прокси** в `server/.env`:
```bash
# Прокси для обхода региональных ограничений OpenAI
HTTPS_PROXY=http://rBD9e6:jZdUnJ@185.68.187.20:8000
HTTP_PROXY=http://rBD9e6:jZdUnJ@185.68.187.20:8000
PROXY_ENABLED=true
```

2. **Установите зависимость на сервере**:
```bash
cd server
npm install https-proxy-agent
```

3. **Или протестируйте прокси**:
```bash
npm run test-proxy
```
...
📖 Подробная инструкция: `PROXY_SETUP.md`
